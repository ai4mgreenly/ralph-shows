import { submitted, navigate, setPage } from "../state.ts";

export function Submitted() {
  const { items, page, total } = submitted.value;
  ...
  return (
    <section>
      <h2>Submitted</h2>
      {items.length === 0 ? (
        <p class="empty">No submitted goals</p>
      ) : (
        <>
          <ul class="goal-list">
            {items.map((g) => (
          ...
          {showPagination && (
            <div class="pagination">
              <button
                disabled={page === 1}
                onClick={() => setPage("submitted", page - 1)}
